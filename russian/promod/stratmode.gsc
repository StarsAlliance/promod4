	...
	for(;;)
	{
		...
		if ( self getCurrentWeapon() == "radar_mp" && ( ( !isDefined(self.inAction) || !self.inAction) && self isOnGround() ) )
		{
			...
			if(isDefined(self.bot))
			{
				...
				{
					self iprintln("Уйдите для возраждения дурака!");
			...
			else
			{
				...
				if(isdefined(newBot))
				{
					...
					self.hint6 setText( "Движение: Нажмите ^3[{+actionslot 1}]" );
				}
				else
					self iprintln("Невозможно добавить бота, сервер полон?");
			}
			self.inAction = false;
		}
...
createHUD()
{
	...
	self.hint1 setText( "Отключить: Удерживайте ^3[{+melee}] ^7+ ^3[{+activate}]" );
	...
	self.hint2 setText( "Стоп: Нажмите ^3[{+attack}]" );
	...
	self.hint3 setText( "Возврат: Нажмите ^3[{+activate}]" );
	...
	self.hint4 setText( "Сохранение: Нажмите ^3[{+melee}] ^7Дважды" );
	...
	self.hint5 setText( "Загрузка: Нажмите ^3[{+activate}] ^7Дважды" );
	...
	if(getDvar("dedicated") == "listen server")
	{
		...
		if(!getDvarInt( "sv_punkbuster" ))
			self.hint6 setText( "Возрождение: Нажмите ^3[{+actionslot 1}]" );
		else
		{
			self.hint6 setText( "Возрождение: Отключен Punkbuster" );
...
createServerHUD()
{
	...
	if(getDvar("dedicated") == "listen server")
	{
		...
		traindummy setText( "Тренировка с Дураками" );
	}
}