...
timeoutCall()
{
	...
	{
		self iprintln("^3Таймаут сейчас не доступен");
		return;
	}
	...
	if ( game["promod_timeout_called"] )
	{
		if ( isDefined( game["promod_timeout_called_by"] ) )
		{
			if ( self == game["promod_timeout_called_by"] )
			{
				...
				self iprintln("^3Таймаут уже заказан " + game["promod_timeout_called_by"].name);
		}
		else
			self iprintln("^3Таймаут уже заказан");
	...
	{
		self iprintln("^3Разрешен только один таймаут на команду/полупериод");
		return;
	}

	game["promod_timeout_called_by"] = self;
	iprintln("^3Таймаут заказан " + self.name);

	if ( !isDefined( level.strat_over ) || level.strat_over )
		self iprintln("^3Запростите таймаут снова для отмены");
